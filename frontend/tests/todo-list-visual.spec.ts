...
test('todo list default visual', async ({ page }) => {
    ...
    await expect(page.locator('.todo-app')).toHaveScreenshot('todo-list-default.png');
});
...
test('todo list empty state visual', async ({ page }) => {
    ...
    await expect(page.locator('.todo-app')).toHaveScreenshot('todo-list-empty-state.png');
});